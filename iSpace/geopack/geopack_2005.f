...
Cf2py intent(in) :: XGSM,YGSM,ZGSM
Cf2py intent(out) :: HXGSM,HYGSM,HZGSM

C   Check if GEOPACk is set up using recalc
      CALL CHECK_RECALC()
...
Cf2py intent(in) :: XGSM,YGSM,ZGSM
Cf2py intent(out) :: BXGSM,BYGSM,BZGSM
...
C================================================================================
c
      SUBROUTINE SPH2CAR (R,THETA,PHI,X,Y,Z)
C
C     CONVERTS SPHERICAL COORDS INTO CARTESIAN ONES 
C
C     AUTHOR:  Xiangning Chu (2022-09-28)
C      
C
...
Cf2py intent(out) :: X,Y,Z
      ...
      CALL SPHCAR (R,THETA,PHI,X,Y,Z,1)
...
C================================================================================
c
      SUBROUTINE CAR2SPH (R,THETA,PHI,X,Y,Z)
C
C     CONVERTS SPHERICAL COORDS INTO CARTESIAN ONES 
C
C     AUTHOR:  Xiangning Chu (2022-09-28)
...
C
      IY=IYEAR
C
C  WE ARE RESTRICTED BY THE INTERVAL 1965-2025, FOR WHICH THE IGRF COEFFICIENTS
...
C    
C   CHECK IF GEOPACK IS INITIALIZED USING RECALC
C    
C     OUTPUT:  FLAG=1 IF GEOPACK IS INITIALIZED, -1 OTHERWISE. 
C 
      COMMON /GEOPACK1/ AA(35)
      COMMON /GEOPACK2/ G(105),H(105),REC(105)

      flag= -1.0
      do n=1, 35
         if (AA(n) .ne. 0.0) flag= 1.0
      end do

      do n=1,105 
          if(G(n) .ne. 0.0) flag= 1.0
          if(H(n) .ne. 0.0) flag= 1.0
          if(REC(n) .ne. 0.0) flag= 1.0
      end do
      
      if (flag == -1.0) then 
            write(*,*), '========== ERROR ============================'
            write(*,*), '      GEOPACK is not initialized.            '
            write(*,*), '   Call geopack_recalc first to set up       '
            write(*,*), '      Program is stopped now                 '
            write(*,*), '========== ERROR ============================'
            stop
      endif
...
c=========================================================================================
      SUBROUTINE GEO2MAG (XGEO, YGEO, ZGEO, XMAG, YMAG, ZMAG)
c     This routine converts GEO to MAG
      COMMON /GEOPACK1/ ST0,CT0,SL0,CL0,CTCL,STCL,CTSL,STSL,AB(19),BB(8)
...
c=========================================================================================
      SUBROUTINE MAG2GEO (XGEO, YGEO, ZGEO, XMAG, YMAG, ZMAG)
c     This routine converts MAG to GEO
      COMMON /GEOPACK1/ ST0,CT0,SL0,CL0,CTCL,STCL,CTSL,STSL,AB(19),BB(8)
...
Cf2py intent(inout) :: XGEI,YGEI,ZGEI
...
c=========================================================================================
      SUBROUTINE GEI2GEO (XGEI,YGEI,ZGEI,XGEO,YGEO,ZGEO)
c     This routine converts GEI to GEO
      COMMON /GEOPACK1/ A(27),CGST,SGST,B(6)
...
c=========================================================================================
      SUBROUTINE GEO2GEI (XGEI,YGEI,ZGEI,XGEO,YGEO,ZGEO)
c     This routine converts GEI to GEO
      COMMON /GEOPACK1/ A(27),CGST,SGST,B(6)
...
Cf2py intent(inout) :: XSM,YSM,ZSM
Cf2py intent(in) :: J
...
c=========================================================================================
      SUBROUTINE MAG2SM (XMAG,YMAG,ZMAG,XSM,YSM,ZSM)
c     This routine converts MAG to SM
      COMMON /GEOPACK1/ A(8),SFI,CFI,B(7),AB(10),BA(8)
...
c=========================================================================================
      SUBROUTINE SM2MAG (XMAG,YMAG,ZMAG,XSM,YSM,ZSM)
c     This routine converts SM to MAG
      COMMON /GEOPACK1/ A(8),SFI,CFI,B(7),AB(10),BA(8)
...
C
Cf2py intent(inout) :: XGSM,YGSM,ZGSM
...
c=========================================================================================
      SUBROUTINE GSM2GSE (XGSM,YGSM,ZGSM,XGSE,YGSE,ZGSE)
c     This routine converts GSM to GSE
      COMMON /GEOPACK1/ A(12),SHI,CHI,AB(13),BA(8)
...
c=========================================================================================
      SUBROUTINE GSE2GSM (XGSM,YGSM,ZGSM,XGSE,YGSE,ZGSE)
c     This routine converts GSE to GSM
      COMMON /GEOPACK1/ A(12),SHI,CHI,AB(13),BA(8)

Cf2py intent(in) :: XGSE,YGSE,ZGSE
Cf2py intent(out) :: XGSM,YGSM,ZGSM
...
c=========================================================================================
      SUBROUTINE SM2GSM (XSM,YSM,ZSM,XGSM,YGSM,ZGSM)
c     This routine converts SM to GSM
      COMMON /GEOPACK1/ A(10),SPS,CPS,B(15),AB(8)
...
c=========================================================================================
      SUBROUTINE GSM2SM (XSM,YSM,ZSM,XGSM,YGSM,ZGSM)
c     This routine converts GSM to SM
      COMMON /GEOPACK1/ A(10),SPS,CPS,B(15),AB(8)
...
c=========================================================================================
      SUBROUTINE GEO2GSM (XGEO,YGEO,ZGEO,XGSM,YGSM,ZGSM)
c     This routine converts GEO to GSM
      COMMON /GEOPACK1/AA(17),A11,A21,A31,A12,A22,A32,A13,A23,A33,D,B(8)
...
c=========================================================================================
      SUBROUTINE GSM2GEO (XGEO,YGEO,ZGEO,XGSM,YGSM,ZGSM)
c     This routine converts GEM to GEO
      COMMON /GEOPACK1/AA(17),A11,A21,A31,A12,A22,A32,A13,A23,A33,D,B(8)
...
C=====================================================================================
C
      SUBROUTINE BFIELD (X,Y,Z,BX,BY,BZ,IOPT,PARMOD,EXNAME,INNAME)
C
...
C      CALL EXNAME (IOPT,PARMOD,PSI,X,Y,Z,BXGSM,BYGSM,BZGSM)
C      CALL INNAME (X,Y,Z,HXGSM,HYGSM,HZGSM)
...
Cf2py intent(in) ::  DS,ERRIN, IOPT,PARMOD
Cf2py intent(in) ::  EXNAME,INNAME
...
C
      ...
      INTEGER L
      REAL DS, DIR, RLIM, R0, XI, YI, ZI, XF, YF, ZF, XX, YY, ZZ, PARMOD
      COMMON /GEOPACK1/ AA(26),DD,BB(8)
C      EXTERNAL EXNAME,INNAME
      CHARACTER EXNAME*(*),INNAME*(*)

Cf2py intent(in) ::  XI, YI, ZI
Cf2py intent(in) ::  DIR, RLIM, R0, IOPT, PARMOD
Cf2py intent(in) ::  EXNAME, INNAME
Cf2py intent(out) :: XF,YF,ZF
Cf2py intent(out) :: XX,YY,ZZ
...
c
  ...
  8   XF=X
      YF=Y
      ZF=Z
C  Revision by X.Chu 2022-09-23
